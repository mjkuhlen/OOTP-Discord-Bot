...
export default new client.command({
    ...
    run: async (client, interaction) => {
        try {
            ...
            const gd = await prisma.games_data.findMany({
                where: {
                    OR: [
                      {
                        home_team_name: interaction.options.getString('name')!,
                      },
                      {
                        away_team_name: interaction.options.getString('name')!,
                      },
                    ],
                    AND: [
                      {
                ...
                select: {
                    ...
                    winningPitcher: {
                        select: {
                            ...
                            _count: {
                                select: {
                                    gamesAsWinningPitcher: { where: { game_type: 0 }},
                                    gamesAsLosingPitcher: { where: { game_type: 0 }}
                                }
                            }
                        },
                    },
